...
public class MyWordle {
  ...
  private Integer outputWidth; // = 1000;
  private boolean allowRotate = true;

  public MyWordle(int width, int height) {
  ...
  public void doLayout() {
    ...
    //      return;
    if (imageSize == null) {
      System.err.println("imageSize == null");
      this.imageSize = new Dimension(1000, 1000);
    }
    ...
    for (Word w : this.words) {
      ...
      if (ff == null)
        ff = this.fontFamily;
      int fontSize = (int) (((w.getWeight() - low) / (high - low)) * (this.biggestSize - this.smallestSize))
          + this.smallestSize;
    ...
    for (int i = 1; i < this.words.size(); ++i) {
      ...
      boolean done = false;
      double radius = 0.5 * Math.min(first.bounds.getWidth(), first.bounds.getHeight());
      while (!done) {
    ...
    this.imageSize = new Dimension((int) (maxx - minx + 0.5), (int) (maxy - miny + 0.5));
  }

  void random() {
    for (int i = 0; i < 50; ++i) {
      String text = DLUtil.RandomWord();
  ...
  void draw(BufferedImage image) {
    ...
    double ratio = outputWidth / imageSize.width;
    ...
    for (Word w : this.words) {
      ...
      if (c != null) {
        ...
        g.setStroke(new BasicStroke(w.getLineHeight(), BasicStroke.CAP_BUTT, BasicStroke.JOIN_ROUND));
        g.setColor(c);
        g.draw(w.shape);
        g.setStroke(old);
      }
  ...
  public void saveAsPNG(File file) throws IOException {
    ...
    Dimension dim = new Dimension(this.imageSize.width, this.imageSize.height);
    if (this.outputWidth != null) {
      double ratio = this.outputWidth / dim.width;
  ...
  public void saveAsSVG(File file) throws IOException, XMLStreamException {
    ...
    w.writeAttribute("width", String.valueOf(this.imageSize.width));
    w.writeAttribute("height", String.valueOf(this.imageSize.height));
  ...
  public void saveAsPostscript(File file) throws IOException {
    PrintWriter out = new PrintWriter(file);
    out.println("%!PS-Adobe-2.0");
    out.println("%%%BoundingBox: 0 0 " + this.imageSize.width + " " + this.imageSize.height);
