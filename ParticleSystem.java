 ...
 */
public class ParticleSystem extends Applet implements Runnable, MouseListener {
	...
	public void init() {
		...
		Gradient g = new Gradient(null, new int[]{0xff0000, 0x0ff000, 0x00ff00, 0x000ff0, 0x0000ff});
		gradients[0] = g;
/*		
		try {
			ObjectInputStream ois = new ObjectInputStream(getClass().getResourceAsStream("gradients.ser"));
			gradients = (Gradient[])ois.readObject();
			ois.close();
		}
		catch (Exception e) {
			e.printStackTrace();
...
*/		
	...
	public void setGradient(int n) {
		...
		byte[] b = new byte[256];
		for (int i = 0; i < 256; i++) {
			long rgb = gradients[n].getColor((float)(i/255.0));
//			System.err.println("get " + (float)(i/255.0) + " " + rgb);
